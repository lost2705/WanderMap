...
0.7 Phase 1 — Real City Boundaries is **PARTIAL**: the provider-neutral subsystem, persistent PostGIS cache, authenticated API and World polygon/fallback rendering are implemented, but no permitted real provider or representative-city coverage has been verified. At World zoom 8+, verified municipal geometry replaces that City's approximate glow; far clusters and Journey country/route/numbered-stop behavior remain unchanged. Unavailable or failed lookups keep glow. Boundary geometry is shared public City metadata; visited selection remains account-specific. See [ADR 0003](docs/adr/0003-real-city-boundaries.md) for source research, semantics, limits, licensing and deferred coverage.
