...
- generate and refine a trip draft, review it, then explicitly create an ordinary Journey with verified places;
...
WanderMap 0.6 adds a read-only Trip Planner on the same bounded `TravelAgent` loop. Natural-language requests produce a schema-constrained, application-owned `TripPlanDraft`, which the backend deserializes and validates before returning it to the SPA. Phase 2 lets the user refine that draft repeatedly with natural-language adjustments; each successful response atomically replaces the editorial itinerary while a failed update leaves the previous draft visible. `New plan` resets the ephemeral browser state. This is an editor workflow, not a chat transcript.

The planner can use the authenticated user's profile, Journeys, Bucket List, short-term weather, and a provider-neutral place-search tool backed by the existing city-search service. Place search resolves names and coordinates; it is not treated as a recommendation ranking. A draft returned by the browser is still untrusted: refinement validates its bounds, omits client sources and personal flags from model context, reacquires Journey and Bucket List evidence, resolves every final stop, validates the complete replacement through the same `TripPlanValidator`, and derives source labels on the backend. Drafts remain ephemeral and the agent remains read-only. Phase 3 adds explicit **Create Journey** after review: a deterministic application service revalidates the draft and canonical places, then creates the Journey without invoking the model. This phase has no bookings, live prices, transport schedules, exact long-range weather, cost calculations, or automatic application of a draft. The decision is recorded in [`docs/adr/0002-structured-trip-planning.md`](docs/adr/0002-structured-trip-planning.md).

Apply persists only the title, optional Journey dates and ordered canonical City stops. Planner prose and day allocations stay in the draft; stop journal dates/notes are not generated, so planner suggestions do not become Memories. Both dates may be null; otherwise inclusive duration must match the draft. Success selects the new Journey on the existing map and refreshes profile/map/Bucket visited data. Bucket List rows are never removed. Draft generation and refinement require AI availability; Apply works with `WANDERMAP_AI_ENABLED=false` and no provider key.

Apply requires a client-generated UUID `requestId`. Flyway V11 stores a small owner-scoped idempotency ledger in the same transaction as the Journey and all stops. Repeating the same key and canonical typed payload returns the original Journey; a changed payload or a deleted original Journey returns `409 CONFLICT`. Hashing normalizes domain names/title, six-decimal coordinates, null/absent optional lists and source-label ordering, while preserving itinerary order and the full validated draft content (including ephemeral prose/flags). Concurrent retries are serialized by the database unique constraint. Failure rolls back the key and all new travel data, so the same request can be retried safely. The frontend retains its key for network/5xx retries; a conflict disables Apply until an explicit refinement/New plan, without automatically generating another key. Successful creation remains successful even if a secondary refresh fails. Old-session responses are ignored. Idempotency survives application restart; the browser's ephemeral draft/key does not survive closing or reloading Planner. Pre-audit local keys still accept their original typed representation; see ADR 0002 for this compatibility limit.

Submitted place names/countries/coordinates must match one existing canonical City or one provider-neutral search result. Persistence uses the existing six-decimal HALF_UP coordinate identity and normalized names; an arbitrary coordinate displacement is not stored. Sub-rounding differences reuse a City; larger differences must match fresh place evidence. Same-name cities remain distinct at different locations. Unresolved combinations return `400 PLACE_UNRESOLVED`; malformed drafts return `400 INVALID_REQUEST`; search outages retain `503 GEOCODING_UNAVAILABLE`, and database failures return retryable `503 APPLY_UNAVAILABLE`. No AI repair or hidden replan occurs.

Enrichment locks the still-unlocated City row so concurrent requests cannot move each other's stops. A race to insert the same new City may still yield a clean retryable `503`, never duplicate identity or partial Journey data. JDBC server error details are disabled to prevent constraint errors from leaking row values into application logs; minimal error categories remain available. Database-server logs and optional SQL bind logging need separate operational privacy controls.
...
0.7 Phase 1 — Real City Boundaries is **DONE** for its validated NL/FR/IT/LI scope: the provider-neutral subsystem, persistent PostGIS cache, authenticated API and World polygon/fallback rendering are implemented, and a pinned local self-hosted Nominatim was used to qualify Amsterdam, Paris, Rome, Vaduz and the Malbun fallback. This is not a worldwide municipality ontology or a production-provider availability claim. At World zoom 8+, verified municipal geometry replaces that City's approximate glow; far clusters and Journey country/route/numbered-stop behavior remain unchanged. Unavailable or failed lookups keep glow. Boundary geometry is shared public City metadata; visited selection remains account-specific. See [ADR 0003](docs/adr/0003-real-city-boundaries.md) for source research, country semantics, limits, licensing and deferred coverage.

Boundary external lookup is **disabled by default in every profile** (`wandermap.city-boundaries.external-enabled=false`), with no default URL or User-Agent. The application and `GET /api/places/{cityId}/boundary` work without an upstream: saved geometry can still be served; otherwise the response provides controlled fallback. To opt in, set `WANDERMAP_CITY_BOUNDARIES_EXTERNAL_ENABLED=true`, `WANDERMAP_CITY_BOUNDARIES_BASE_URL` to a permitted self-hosted/third-party Nominatim-compatible API base, and an identifying `WANDERMAP_CITY_BOUNDARIES_USER_AGENT` with operator contact. Public `nominatim.openstreetmap.org` is rejected, not used as a local/demo default. Review the chosen service's terms/capacity before enabling; no service/account is provisioned by this milestone.

Flyway V12 stores original and topology-preserving display MultiPolygons in EPSG:4326 per canonical City. Name/country checks and PostGIS point coverage validate matches; unsupported, invalid, ambiguous or excessively complex geometry is never replaced with fake polygons. Positive cache lasts 30 days, no-match cache 1 day, temporary failure 5 minutes; failed refresh retains verified geometry. Only visible cities are loaded lazily, with two concurrent frontend requests and bounded geometry/render budgets. OSM boundary data is ODbL and supplies its own MapLibre attribution entry linking to the [OSM copyright/license page](https://www.openstreetmap.org/copyright); basemap attribution is not implicitly assumed to cover it. No live boundary provider is called in CI.
...
- `POST /api/ai/trip-plan/refine` — refine an untrusted current draft with a natural-language instruction and return a fully revalidated replacement (same message limit).
- `POST /api/ai/trip-plan/apply` — explicitly create a Journey from `{plan: TripPlanDraft, requestId: UUID}`; returns the existing Trip DTO with `200` for creation or idempotent replay. Requires auth and CSRF, but no AI provider.
...
curl -X POST http://localhost:8080/api/ai/trip-plan/refine \
  -H "Content-Type: application/json" \
  -d '{"plan":{"...":"current TripPlanDraft"},"message":"Use fewer cities and make it more relaxed."}'
...
The 0.6 planner requests strict JSON Schema output through the existing provider-neutral model port and adds only one tool, `search_places`. Initial planning uses `wander-map-trip-planner-v1`; refinement uses `wander-map-trip-planner-refinement-v1` with the same schema, tools, stateless Responses API continuation mechanics, `store=false`, and output-token bound. Its `daysAtStop` values must sum to `durationDays`; concrete start/end dates use inclusive calendar-day semantics and must agree with that duration, while unspecified dates remain null. Plans are limited to 1–60 days, 1–12 resolved unique places, at most 8 activities per stop, and at most 10 considerations. Country codes, coordinates, strings, list sizes, dates, personal-history flags, and place-resolution evidence are validated application-side in addition to the provider schema. Invalid model output becomes the controlled `AI_INVALID_PLAN` response and is never exposed as raw JSON. Invalid client drafts are rejected with `400 INVALID_REQUEST` before provider invocation. Exact future weather remains unavailable beyond the current forecast horizon.

Deferred planner work includes persisted TripPlan drafts, a manual drag/drop planner editor, RU/EN localization, live costs/transport/hotel data, historical climate, route tools, RAG or long-term AI memory, and the existing platform debt around bundle splitting, permitted city-boundary provider/coverage verification, full-world framing, premium basemap polish, antimeridian routing, and a raw AI HTTP byte ceiling. Idempotency keys/tombstones currently have no expiry; retention administration is deferred. Native mobile applications remain after the working web application is complete; the preferred future direction is Capacitor over the existing React/TypeScript frontend and Spring backend, with no mobile-specific architecture introduced now.
