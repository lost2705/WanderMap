...
Stage C adds no cache-schema or semantic-version column. Existing negative results expire after one day and are safely recomputed under the new policy; a deployment may invalidate selected negative rows earlier when immediately qualifying a new policy. Positive geometry remains valid because it already passed trusted identity and GIS checks and is more valuable than eager invalidation. Stage C real-provider qualification uses only targeted local negative-cache resets for the cities under test. No broad cache clear or migration is required.
...
The unchanged PostGIS persistence stored all three as EPSG:4326 MultiPolygons, `ST_IsValid=true` and `ST_Covers(canonical City point)=true`. Second requests added no provider call, proving persistent positive-cache reuse. Only each test City's local cache row was invalidated before qualification. Stage C adds no migration or semantic-version column: one-day negative entries expire naturally, while operators may use a targeted negative-row invalidation when immediately qualifying a new policy. Positive cached geometry is preserved.
